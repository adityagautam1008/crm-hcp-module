...
def create_interaction(payload: InteractionCreate, db: Session = Depends(get_db)):
    ...
    # If no hcp_id provided, try to find HCP by name, or create a new one
    if not hcp_id and payload.hcp_name:
        existing_hcp = db.query(HCP).filter(
            HCP.name.ilike(payload.hcp_name.strip())
        ).first()

        if existing_hcp:
            hcp_id = existing_hcp.id
        else:
            # Auto-create new HCP with just the name
            new_hcp = HCP(name=payload.hcp_name.strip())
            db.add(new_hcp)
            db.flush()  # gets the new ID without committing yet
            hcp_id = new_hcp.id
